...
void BasePhyLayer::handleAirFrameStartReceive(AirFrame* frame) {
	...
	if(usePropagationDelay) {
		...
		if(simTime()>19){
		simtime_t delay = simTime()+ 0.0 - s.getSendingStart();
		s.setPropagationDelay(delay);
		}
		else{
		 simtime_t delay = simTime()+ 0 - s.getSendingStart();
		s.setPropagationDelay(delay);
		}
		//cout << "prop delay" << s.getPropagationDelay()<<endl;
	}
	Signal& s = frame->getSignal();
//	cout << "prop delay" << s.getPropagationDelay()<<endl;
	if(simTime()>19)
	assert(frame->getSignal().getReceptionStart() == simTime()+ 0.0);
	else
	    assert(frame->getSignal().getReceptionStart() == simTime()+ 0);
