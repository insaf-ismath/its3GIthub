...
simtime_t Decider80211p::processNewSignal(AirFrame* msg) {
	...
	if (recvPower < sensitivity) {
		...
		if (phy11p->getRadioState() == Radio::TX) {
			...
			//std::cout << "AirFrame: " << frame->getId() << " (" << recvPower << ") received, while already sending. Setting BitErrors to true" << std::endl;
			...
			if (!currentSignal.first) {
				...
				//std::cout<< "AirFrame: " << frame->getId() << " with (" << recvPower << " > " << sensitivity << ") -> Trying to receive AirFrame." << std::endl;
			}
			else {
				//NIC is currently trying to decode another frame. this frame will be simply treated as interference
				//std::cout<< "AirFrame: " << frame->getId() << " with (" << recvPower << " > " << sensitivity << ") -> Already synced to another AirFrame. Treating AirFrame as interference." << std::endl;
			}
...
simtime_t Decider80211p::processSignalEnd(AirFrame* msg) {
	...
	if (frame->getUnderSensitivity()) {
		...
		//std::cout << "not detected by radio card" << std::endl;
	}
	...
	if (result->isSignalCorrect()) {
		...
		if (frame->getUnderSensitivity()) {
			//std::cout << "packet was not detected by the card. power was under sensitivity threshold\n";
		}
		else if (whileSending) {
			//std::cout << "packet was received while sending, sending it as control message to upper layer\n";
			phy->sendControlMsgToMac(new cMessage("Error",RECWHILESEND));
		}
		else {
			//std::cout << "packet was not received correctly, sending it as control message to upper layer\n";
