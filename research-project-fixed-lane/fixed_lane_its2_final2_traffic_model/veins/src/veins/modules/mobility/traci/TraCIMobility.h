...
class TraCIMobility : public BaseMobility
{
    ...
    simtime_t lastUpdate; /**< updated by nextPosition() */
		...
		virtual TraCICommandInterface::Lane* getLaneCommandInterface() const {
		            if (!laneCommandInterface) laneCommandInterface = new TraCICommandInterface::Lane(getCommandInterface()->lane("1to2_0"));
