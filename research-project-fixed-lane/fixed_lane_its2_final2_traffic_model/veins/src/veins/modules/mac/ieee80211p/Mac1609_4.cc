...
//    cout << "inMacLayer" <<endl;
	BaseMacLayer::initialize(stage);
	if (stage == 0) {
//	    cout<< "in stage = 0"<< endl;
...
void Mac1609_4::handleSelfMsg(cMessage* msg) {
	if (msg == nextChannelSwitch) {
		...
		//std::cout << "MacEvent received. Trying to send packet with priority" << lastAC << std::endl;
		...
		//std::cout << "Sending duration will be" << sendingDuration << std::endl;
		if ((!useSCH) || (timeLeftInSlot() > sendingDuration)) {
			...
			//std::cout << "Sending a Packet. Frequency " << freq << " Priority" << lastAC << std::endl;

			sendDelayed(mac, RADIODELAY_11P, lowerLayerOut);  //delay sending through NIC
			if(simTime()>0)
			statsSentPackets++;
		}
...
void Mac1609_4::handleUpperMsg(cMessage* msg) {
    DBG_MAC << "message reached MAC layer-simTime" << simTime() << std::endl;
	WaveShortMessage* thisMsg;
	if ((thisMsg = dynamic_cast<WaveShortMessage*>(msg)) == NULL) {
		error("WaveMac only accepts WaveShortMessages");
	}

	t_access_category ac = mapPriority(thisMsg->getPriority());  // get the access category

	DBG_MAC << "Received a message from upper layer for channel "
	        << thisMsg->getChannelNumber() << " Access Category (Priority):  "
	        << ac << std::endl;
	/*std::cout << "Received a message from upper layer for channel "
	            << thisMsg->getChannelNumber() << " Access Category (Priority):  "
	            << ac << std::endl;*/

	t_channel chan;     // which channel CH or SCH
	...
	int num = myEDCA[chan]->queuePacket(ac,thisMsg);    // find out the place in the queue

	//packet was dropped in Mac = queue full
	if (num == -1) {
		statsDroppedPackets++;
		return;
	}
	...
	//std::cout << "sorted packet into queue of EDCA " << chan << " this packet is now at position: " << num << std::endl;
	...
	if (num == 1 && idleChannel == true && chan == activeChannel) {
		...
		//std::cout << "finished contention" << std::endl;
		...
		if (nextEvent != -1) {
			if ((!useSCH) || (nextEvent <= nextChannelSwitch->getArrivalTime())) {
				...
				//std::cout << "Updated nextMacEvent:" << nextMacEvent->getArrivalTime().raw() << std::endl;
			}
			else {
	...
	if (num == 1 && idleChannel == false && myEDCA[chan]->myQueues[ac].currentBackoff == 0 && chan == activeChannel) {
		...
		DBG_MAC << "channel not idle so backoff" << std::endl;
	}

}

void Mac1609_4::handleLowerControl(cMessage* msg) {
    /** TX_OVER Indicates the end of a send transmission. */
	if (msg->getKind() == MacToPhyInterface::TX_OVER) {

		DBG_MAC << "Successfully transmitted a packet on " << lastAC << std::endl;
//		cout << "message transmitted at simtime sim time" << simTime() << endl;
...
simtime_t Mac1609_4::EDCA::startContent(simtime_t idleSince,bool guardActive) {
	...
	//std::cout << "Restarting contention." << std::endl;

	simtime_t nextEvent = -1;

	simtime_t idleTime = SimTime().setRaw(std::max((int64_t)0,(simTime() - idleSince).raw()));;  // idle time is max of 0 and idle since

	lastStart = idleSince;

	DBG_MAC << "Channel is already idle for:" << idleTime << " since " << idleSince << std::endl;
	//std::cout << "Channel is already idle for:" << idleTime << " since " << idleSince << std::endl;
	...
	for (std::map<t_access_category, EDCAQueue>::iterator iter = myQueues.begin(); iter != myQueues.end(); iter++) {
		if (iter->second.queue.size() != 0) {
			...
			if (guardActive == true && iter->second.currentBackoff == 0) {
				//cw is not increased
				iter->second.currentBackoff = OWNER intuniform(0,iter->second.cwCur);
//				std:cout << "current_backoff -" << iter->second.currentBackoff << std::endl;
				if(simTime()>0)
				statsNumBackoff++;
				DBG_MAC << "numbackoff-" << statsNumBackoff << std::endl;
			}
...
//	cout << "Queue " << ac << " will go into post-transmit backoff for " << myQueues[ac].currentBackoff << " slots" << endl;
