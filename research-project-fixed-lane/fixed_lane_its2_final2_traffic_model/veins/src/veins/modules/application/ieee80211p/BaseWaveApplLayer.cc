...
void BaseWaveApplLayer::initialize(int stage) {
	...
	if (stage==0) {
		...
		if (sendBeacons) {
			scheduleAt(simTime() + offSet+0.0, sendBeaconEvt);
		}
...
WaveShortMessage*  BaseWaveApplLayer::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, int serial) {
    ...
    if (name == "want_change") {
        DBG << "Creating want_change with Priority " << priority << " at Applayer at " << wsm->getTimestamp() << std::endl;
    }
    if (name == "dont_change") {
        DBG << "Creating dont_change with Priority " << priority << " at Applayer at " << wsm->getTimestamp() << std::endl;
    }
